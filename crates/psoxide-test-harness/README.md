...
| `access-time` | yes (`Done.`) | **cycle-accurate** per-region access timing; the one-cycle-per-instruction model cannot reproduce the reference cycle counts (the `access_time_reproduces_region_timing` gate parses the cycles-per-access table and asserts each region against the `psx.log` golden within ±1.5 cycles). |
